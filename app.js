...
async function init() {
  const resp = await fetch('data/satellites.json');
  satellites = (await resp.json()).map(s => ({
    ...s,
...
init().catch(err => {
  detailsEl.textContent = `Failed to load catalog: ${err.message}. Run scripts/update_data.py to build data/satellites.json.`;
});
