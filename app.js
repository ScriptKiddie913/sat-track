...
function propagate(sat, at) {
  ...
  const cosO = Math.cos(sat.raan),
    sinO = Math.sin(sat.raan);
  const cosI = Math.cos(sat.inc),
    sinI = Math.sin(sat.inc);
  const cosW = Math.cos(sat.argPerigee),
    sinW = Math.sin(sat.argPerigee);
  ...
  const alt =
    Math.sqrt(xe * xe + ye * ye + ze * ze) - EARTH_RADIUS_KM;
...
function renderTable() {
  ...
  for (const sat of filtered.slice(0, 400)) {
    ...
    tr.innerHTML = `
      <td>${sat.name}</td>
      <td>${sat.country || 'Unknown'}</td>
      <td>${sat.type || 'Unknown'}</td>
      <td>${sat.orbitClass}</td>
    `;
...
searchEl.addEventListener('input', () => {
  ...
  filtered = !q
    ? satellites
    : satellites.filter((s) =>
        [s.name, s.country, s.type, s.orbitClass, String(s.norad)]
          .join(' ')
          .toLowerCase()
          .includes(q)
      );
